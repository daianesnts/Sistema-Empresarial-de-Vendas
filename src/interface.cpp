...
bool tratarErroVenda(int cod) {
    ...
    if (cod == ERRO_NADA_ESCRITO) {
        const char *texto = "ERRO: Preencha todas as informacoes";
        gotoxy(11, 28);
        printf(texto);
    } else if (cod == ERRO_PRODUTO_NAO_EXISTE) {
        const char *texto = "ERRO: ID invalido";
        gotoxy(11, 28);
        printf(texto);
    } else if (cod == ERRO_QUANTIDADE_INSUFICIENTE) {
        const char *texto = "ERRO: Quantidade em estoque insuficiente";
        gotoxy(11, 28);
        printf(texto);
    } else if (cod == ERRO_QTD_INVALIDA) {
        const char *texto = "ERRO: Quantidade não pode ser zero";
        gotoxy(11, 28);
        printf(texto);
    } else if (cod == PRODUTO_VALIDO) {
        textcolor(GREEN);
        const char *texto = "Produto inserido na fila de vendas";
        gotoxy(11, 28);
        printf(texto);
        prosseguir = true;
    }
...
bool tratarErroCad(int cod) {
    ...
    textcolor(RED);
    if (cod == ERRO_NADA_ESCRITO) {
        const char *texto = "ERRO: Preencha todas as caixas de texto";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
    } else if (cod == ERRO_CODIGO_EXISTENTE) {
        const char *texto = "ERRO: Codigo/ID existente";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
    } else if (cod == ERRO_NUMERO_INVALIDO) {
        const char *texto = "ERRO: Insira o numero corretamente";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
    } else if (cod == SEM_ERRO) {
        textcolor(GREEN);
        const char *texto = "Fornecedor cadastrado com sucesso!";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
        prosseguir = true;
...
bool tratarErroCadP(int cod) {
    ...
    if (cod == ERRO_NADA_ESCRITO) {
        ...
        const char *texto = "ERRO: ID existente";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
    } else if (cod == SEM_ERRO) {
        textcolor(GREEN);
        const char *texto = "Produto cadastrado com sucesso!";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
        prosseguir = true;
...
bool tratarErroEdicao(int cod) {
    ...
    if (cod == ERRO_NADA_ESCRITO) {
        ...
        textcolor(GREEN);
        const char *texto = "Fornecedor editado com sucesso!";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
        prosseguir = true;
...
void logicaListaCliente(int *opcao, int *desliza, int linhas, int total, bool *enter, bool *esc) {
    ...
    if (tecla == TECLA_ESCAPE) {
        ...
        case SETA_BAIXO:
            (*opcao)++;
            if (*opcao >= linhas) {  // Verifica se a opção ultrapassou o número total de clientes
                *opcao = linhas - 1;  // Impede que a opção ultrapasse o número total de clientes
                if ((*opcao + *desliza + 1) < total)
...
void inicializarListaClientes(ListaCliente **lista, Cliente *clientes, int &n) {
    ...
    // Cria uma lista com capacidade de suportar até 20 linhas
...
// Botões de Editar, Excluir e Voltar no centro da tela.
...
bool tratarErroCadC(int cod) {
    ...
    if (cod == ERRO_NADA_ESCRITO) {
        ...
        const char *texto = "ERRO: Insira o CPF corretamente";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
    } else if (cod == SEM_ERRO) {
        textcolor(GREEN);
        const char *texto = "Cliente cadastrado com sucesso!";
        int tam = strlen(texto);
        gotoxy(c-tam, 26);
        printf(texto);
        prosseguir = true;
...
void desenharLista(const ListaCliente *lista, int n) {
    ...
    // Limita a iteração até o número real de clientes (n)
    for (int i = 0; i < n; i++) {
        int corFundo = lista[i].corFundo;
        int corLetra = lista[i].corLetra;
        ...
        for (int i = bytes; i < 11; i++)  // Preenche o espaço restante até 11 caracteres
            printf(" ");

        // Desenha o nome do cliente
        gotoxy(14, lista[i].lin);
        bytes = printf("%s", lista[i].cliente->nome);
        for (int i = bytes; i < 53; i++)  // Preenche o espaço restante até 53 caracteres
            printf(" ");

        // Desenha o CPF do cliente
        gotoxy(67, lista[i].lin);
        bytes = printf("%s", lista[i].cliente->cpf);
        for (int i = bytes; i < 50; i++)  // Preenche o espaço restante até 50 caracteres
            printf(" ");
    }
}
...
/*
 * Limpa memória e esvazia o conteúdo de uma caixa de texto
 */

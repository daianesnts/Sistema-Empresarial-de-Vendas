//NOTA: Este modulo foi originalmente implementado inteiramente dentro do
//header relatorio_vendas.h. O codigo foi reorganizado para o padrao
//correto de separacao declaracao (.h) / implementacao (.cpp).
...
// Variaveis globais do modulo (definicao) 
...
// Ler dados do arquivo vendas.txt e armazenar em um array de strings
...
// Converter os dados lidos do arquivo vendas.txt e armazenar em InfoVenda e InfoItens
void converteDados(string vendasCadastradas[], int linhas, InfoVenda &vendas, InfoItens itens[], int &numItens, DadosRelatorio &dado) {
    ...
    for (int i = 0; i < numVendas; i++) {
        int baseIndice = 5 + i * 5; // Indice base para cada venda

        itens[numItens].id = stoi(vendasCadastradas[baseIndice]);
        strncpy(itens[numItens].nome, vendasCadastradas[baseIndice + 1].c_str(), TAMANHO_NOME_ITEM);
        itens[numItens].nome[TAMANHO_NOME_ITEM] = '\0';
...
// Carregar dados do arquivo vendasTotais.txt e armazenar em DadosRelatorio
void carregarDadosRelatorio(DadosRelatorio &dado) {
    ifstream leitura(vendasTotais);
    if (!leitura.is_open()) {
        return; // Se o arquivo nao existir, mantem os valores padrao
    }
...
void vendasTotaisRel(DadosRelatorio &dado, InfoVenda &vendas, InfoItens itens[], int numItens) {
    ...
    saida << dado.quantidadeTotal << endl
          << dado.receitaTotal << endl
          << dado.receitaTotal / dado.quantidadeTotal << endl
          << dado.maisVendido << endl
          << dado.maisUnidades << endl
          << dado.menosVendido << endl
          << dado.menosUnidades << endl;
    saida.close();
}
...
bool conversaoData(const string &dataStr, tm &dataTm) {
    ...
    dataTm.tm_mon -= 1;     // Meses sao 0-11 em struct tm
...
// Ler o arquivo de vendas listadas e preencher o array vendasListadas
void lerVendasLista() {
    ...
    while (getline(arquivo, linha)) {
        if (totalVendas >= MAX_ITENS) {
            cerr << "Aviso: Limite maximo de vendas atingido (" << MAX_ITENS << ")" << endl;
            break;
        }
...
// Gerar o relatorio de vendas por intervalo de datas
bool gerarRelatorioIntervalo(string dataInicioStr, string dataFimStr) {
    ...
    if (!encontrouVendas) {
        ...
        saida << "---------------------------------------------------------" << endl;
        saida << "Unidades vendidas nesse periodo: " << totalUnidades << " unidades" << endl;
...
// Exibir o relatorio de vendas totais na tela
void exibirTotal() {
    ifstream leitura(vendasTotais);
    string s;
...
// Exibir o relatorio de vendas listadas na tela (com scroll)
void exibirLista(const char* arquivo) {
    ...
    for (int i = 0; i < linhas; i++) {
        ...
        resto = i % 6;
        if      (resto == 0) data[p] = s;
        else if (resto == 1) ID[p] = s;
        else if (resto == 2) nomeProduto[p] = s;
        else if (resto == 3) preco[p] = s;
        else if (resto == 4) unidades[p] = s;
        else if (resto == 5) { precoTotal[p] = s; p++; }
    }
    leitura.close();
...
// Exibir o relatorio de vendas por intervalo na tela (com scroll)
...
// Processar e salvar os relatorios de vendas totais e listados para a venda atual
